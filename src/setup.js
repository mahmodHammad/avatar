...
// ----------------------------------------------> camera
const camera = new THREE.PerspectiveCamera(50, 1, 0.001, 700);
camera.position.z = 6;
...
const ssaoPass = new SSAOPass(scene, camera, 90, 90);
ssaoPass.kernelRadius = 8;
ssaoPass.minDistance=0.004
ssaoPass.maxDistance=0.004
...
function addGUI() {
  ...
  gui.add(ssaoPass, "kernelRadius").min(0).max(100);
  gui.add(ssaoPass, "minDistance").min(0.0001).max(0.02);
  gui.add(ssaoPass, "maxDistance").min(0.001).max(0.2);
}
addGUI();
export {
  sceneSetup,
